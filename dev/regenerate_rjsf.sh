...
#
# Run make build first if you've changed ext-apn tasks.
# Run this anytime you've only changed spec.yaml or rjsf-overrides.
#
# NOTE: this does NOT regenerate dags/run_sequential.py. Structural spec.yaml
# changes (new/removed steps, changed partial values, mapvalues/map wiring)
# need a full `make compile` to take effect in the executable DAG.

set -e

GENERATED_DIR="ecoscope-workflows-vehicle-workflow"
REGISTRY_BIN="${GENERATED_DIR}/.pixi/envs/default/bin/wt-registry"
REGISTRY_JSON="/tmp/wt_registry_apn_vehicle.json"
...
time "$REGISTRY_BIN" --format json \
    ...
    --package ecoscope_workflows_ext_apn.tasks \
    > "$REGISTRY_JSON"
...
out = Path('$GENERATED_DIR') / 'ecoscope_workflows_vehicle_workflow' / 'rjsf.json'
